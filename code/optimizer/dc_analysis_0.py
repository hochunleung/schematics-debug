...
def solve_dc(netlist, models, config_dc):
    ...
    for mostype in ['NMOS', 'PMOS']:
        ...
        for m in mosfets[mostype]:
            ...
            if m['name'] in saved_op:
                op_config['saved_op'][m['name']] = {}
                for op in saved_op[m['name']]:
                    op_config['saved_op'][m['name']][op] = dict_models[m['model']].lookup_by_vgs(op, m['params']['l'], VGS, VDS)
