...
class MosData:
    def __init__(self, csv_path, W_ref=4e-6, is_pmos=False):
        ...
        # === 新增: 處理 Noise 數據 ===
        # if 'S_10Hz' in self.df.columns:
        # 避免負值或零
        noise_epsilon = 1e-40
        self.df['S_10Hz'] = self.df['S_10Hz'].clip(lower=noise_epsilon)
        self.df['S_1kHz'] = self.df['S_1kHz'].clip(lower=noise_epsilon)
        self.df['S_10GHz'] = self.df['S_10GHz'].clip(lower=noise_epsilon)
        
        # 正規化為單位寬度的雜訊 PSD (A^2/Hz / um)
        self.df['S_10Hz_W'] = self.df['S_10Hz'] / W_ref
        self.df['S_1kHz_W'] = self.df['S_1kHz'] / W_ref
        self.df['S_10GHz_W'] = self.df['S_10GHz'] / W_ref
    ...
    def get_noise_psd(self, L, VGS, VDS, target_W, f):
        """
        給定工作點與目標尺寸，計算任意頻率 f 的電流雜訊 PSD (A^2/Hz)。
        支援純量 (Scalar) 或 Numpy 陣列 (Array) 輸入。
        """
        # 構造座標點
        pts = np.column_stack((np.atleast_1d(L), np.atleast_1d(VGS), np.atleast_1d(VDS)))
        
        # 1. 從 LUT 內插取得「單位寬度」的雜訊數據
        s_10g_w = self.interps['S_10GHz_W'](pts)
        s_1k_w  = self.interps['S_1kHz_W'](pts)
        s_10_w  = self.interps['S_10Hz_W'](pts)
        
        # 2. 縮放至目標寬度 target_W
        s_10g = s_10g_w * target_W
        s_1k  = s_1k_w * target_W
        s_10  = s_10_w * target_W
        
        # 3. 提取純熱雜訊與純閃爍雜訊
        thermal = s_10g
        flicker_10 = np.maximum(0, s_10 - thermal)
        flicker_1k = np.maximum(0, s_1k - thermal)
        
        # 4. 動態計算閃爍雜訊斜率 EF (防呆處理避免 log(0))
        # EF = log(S_10 / S_1k) / log(100)
        valid_mask = (flicker_10 > 1e-35) & (flicker_1k > 1e-35)
        EF = np.ones_like(flicker_10) # 預設斜率為 1.0
        EF[valid_mask] = np.log(flicker_10[valid_mask] / flicker_1k[valid_mask]) / np.log(100.0)
        
        # 限制 EF 的合理範圍，避免數值發散
        EF = np.clip(EF, 0.5, 1.5)

        # 5. 計算目標頻率 f 的總雜訊
        f_array = np.atleast_1d(f)
        # 如果 f=0 (DC Mismatch 評估等極端情況)，則令 1/f 雜訊為 0
        flicker_total = np.where(f_array > 0, flicker_10 * ((10.0 / f_array) ** EF), 0)
        
        total_psd = flicker_total + thermal
        #print(f"熱雜訊: {thermal[0]:.2e} A^2/Hz | 10Hz閃爍雜訊: {flicker_10[0]:.2e} | 斜率 EF: {EF[0]:.3f}")
        # 如果輸入是單一數值，就回傳單一數值，維持介面整潔
        return total_psd[0] if len(total_psd) == 1 else total_psd
...
if __name__ == "__main__":
    ...
    psd = nch.get_noise_psd(100e-9, 443.3e-3, 443.3e-3, 4e-6, 1e6)
    pprint(psd)