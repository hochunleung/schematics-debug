...
# 測試條件
V_AMP_IN = 1e-3        # 100mV
R_LOAD = 1e6             # 1M (拿掉負載效應可註解掉 build_Y_matrix 裡的 YL)
FIN = 53 / 512 * 100e6   # 10.35 MHz

# ================= 2. 高頻寄生電容 (Const 設置) =================
# 請替換為 Cadence dcOp 實際提取出的準確數值
C_GS2 = 2.339e-15          # M2 cgs (影響高頻前饋)
C_GS3 = 2.441e-15          # M3 cgs (影響高頻回饋)
C_DB2 = 2.735e-15           # M2 cdb (因 Source-Body 短接，變成跨接 Drain-Source)
C_DB3 = 2.888e-15           # M3 cdb

C_TAIL_GND = 3.074e-15                                  # 尾節點 vs 對地總電容 (M0 cdd + Deep N-Well cjsub)
C_M_GND = 4.475e-15 + 2.241e-15 + 5.67e-15 + 5.645e-15  # 鏡像節點總電容 (M2 Cdd + M4 Cdd + M4/M5 Cgg)
C_OUT_GND = 2.103e-15 + 4.671e-15                       # 輸出節點總電容 (M3 Cdd + M5 Cdd)
# ============================================================
...
def build_Y_matrix(K, RL, omega):
    """建立 3x3 一階線性導納矩陣 (包含完整耦合寄生電容)"""
    Y = np.zeros((3, 3), dtype=complex)
    YL = 1.0 / RL
    s = 1j * omega
    
    # 節點 0: vs (尾節點) - 加入對地與跨接寄生電容
    Y[0, 0] = K['M0']['K01'] + K['M2']['K10'] + K['M2']['K01'] + K['M3']['K10'] + K['M3']['K01'] \
              + s*C_TAIL_GND + s*C_GS2 + s*C_GS3 + s*C_DB2 + s*C_DB3
    Y[0, 1] = -K['M2']['K01'] - s*C_DB2
    Y[0, 2] = -K['M3']['K10'] - K['M3']['K01'] - s*C_GS3 - s*C_DB3
    
    # 節點 1: vm (鏡像節點)
    Y[1, 0] = -K['M2']['K10'] - K['M2']['K01'] - s*C_DB2
    Y[1, 1] = K['M2']['K01'] + K['M4']['K10'] + K['M4']['K01'] + s*C_M_GND + s*C_DB2
    Y[1, 2] = 0
    
    # 節點 2: vout (輸出節點)
    Y[2, 0] = -K['M3']['K10'] - K['M3']['K01'] - s*C_GS3 - s*C_DB3
    Y[2, 1] = K['M5']['K10']
    Y[2, 2] = K['M3']['K10'] + K['M3']['K01'] + K['M5']['K01'] + s*C_OUT_GND + s*C_GS3 + s*C_DB3 # + YL
    return Y
...
def run_volterra_analysis():
    ...
    Y1 = build_Y_matrix(K_dict, R_LOAD, w)
    gm2 = K_dict['M2']['K10']
    
    # 加上輸入信號經過 C_gs2 產生的前饋位移電流
    i_feedforward_vs = s * C_GS2 * V_AMP_IN
    
    I_in1 = np.array([gm2 * V_AMP_IN + i_feedforward_vs, -gm2 * V_AMP_IN, 0])
    ...
    Y2 = build_Y_matrix(K_dict, R_LOAD, 2 * w)
    ...
    Y3 = build_Y_matrix(K_dict, R_LOAD, 3 * w)
