...
# 測試條件
V_AMP_IN = 10e-3        # 100mV
R_LOAD = 1e6             # 1M
FIN = 53 / 512 * 100e6   # 10.35 MHz

# 估計寄生電容 (你可以先用這些合理估計值，之後再從 Cadence OP 精準提取)
C_TAIL = 3.074e-15 #+ 4.415e-15 + 4.421e-15          # 尾節點總電容 (M0 Cdd + M2/M3 Csb)
C_M = 4.475e-15 + 2.241e-15 + 5.67e-15 + 5.645e-15  # 鏡像節點總電容 (M2 Cdd + M4 Cdd + M4/M5 Cgg)
C_OUT = 2.103e-15 + 4.671e-15                       # 輸出節點總電容 (M3 Cdd + M5 Cdd)
# ============================================================
...
def build_Y_matrix(K, RL, omega):
    """建立 3x3 一階線性導納矩陣 (複數高頻版)"""
    Y = np.zeros((3, 3), dtype=complex) # 改為 complex
    YL = 1.0 / RL
    s = 1j * omega
    ...
    # 節點 0: vs (尾節點) - 加入 Y_tail
    Y[0, 0] = K['M0']['K01'] + K['M2']['K10'] + K['M2']['K01'] + K['M3']['K10'] + K['M3']['K01'] + Y_tail
    Y[0, 1] = -K['M2']['K01']
    Y[0, 2] = -K['M3']['K10'] - K['M3']['K01']
    
    # 節點 1: vm (鏡像節點) - 加入 Y_m
    Y[1, 0] = -K['M2']['K10'] - K['M2']['K01']
    Y[1, 1] = K['M2']['K01'] + K['M4']['K10'] + K['M4']['K01'] + Y_m
    Y[1, 2] = 0
    
    # 節點 2: vout (輸出節點) - 加入 Y_out
    Y[2, 0] = -K['M3']['K10'] - K['M3']['K01']
    Y[2, 1] = K['M5']['K10']
    Y[2, 2] = K['M3']['K10'] + K['M3']['K01'] + K['M5']['K01'] + Y_out # + YL
    return Y
...
def run_volterra_analysis():
    ...
    Y2 = build_Y_matrix(K_dict, R_LOAD, 2 * w) # 靜態下 Y2 與 Y1 相同
