...
def visualize_derivatives():
    ...
    axs[0].plot(vin * 1000, vout * 1000, label='Vout (mV)', color='blue')
    axs[0].set_ylabel('Vout (mV)')
    axs[0].legend(loc='upper right')
    axs[0].grid(True, linestyle='--', alpha=0.6)
    
    axs[1].plot(vin * 1000, d1, label='d1 (a1, Gain)', color='orange')
    axs[1].set_ylabel('d1 (V/V)')
    axs[1].legend(loc='upper right')
    axs[1].grid(True, linestyle='--', alpha=0.6)
    
    axs[2].plot(vin * 1000, d2, label='d2 (2*a2)', color='green')
    axs[2].set_ylabel('d2 (1/V)')
    axs[2].legend(loc='upper right')
    axs[2].grid(True, linestyle='--', alpha=0.6)
    
    axs[3].plot(vin * 1000, d3, label='d3 (6*a3)', color='red')
    axs[3].set_ylabel('d3 (1/V^2)')
    axs[3].set_xlabel('Vin (mV)')
    axs[3].legend(loc='upper right')
    axs[3].grid(True, linestyle='--', alpha=0.6)
    
    plt.suptitle("DC Transfer Characteristic & Its Derivatives", fontsize=14)
    plt.tight_layout()
    plt.show()
